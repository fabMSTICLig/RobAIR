...
private:
    ...
    int nb_possible_association[1000];
    int possible_association[1000][1000];
    float dist_association[1000][1000];
    int best_association[1000];
    int nb_association;
...
void initialize_association() {

    position = 0;
    best_uncertainty = 0;
    for (int loop_leg_tracked=0; loop_leg_tracked<nb_legs_tracked; loop_leg_tracked++) {
        best_association[loop_leg_tracked] = unassociated;
        //current_association[loop_leg_tracked] = unassociated;
        best_uncertainty += leg_uncertainty[loop_leg_tracked];
        index[loop_leg_tracked] = 0;
    }
    nb_association = 0;

    ROS_INFO("init_score: %f", best_uncertainty);
    int observation_detected[1000];
    int corresponding_track[1000];
    for ( int loop_leg_detected=0; loop_leg_detected<nb_legs_detected; loop_leg_detected++)
        observation_detected[loop_leg_detected] = 0;

    ROS_INFO("check possible association");
    //nb_pts = 0;
    for (int loop_leg_tracked=0; loop_leg_tracked<nb_legs_tracked; loop_leg_tracked++) {
        nb_possible_association[loop_leg_tracked] = 1;
        possible_association[loop_leg_tracked][0] = unassociated;
        dist_association[loop_leg_tracked][0] = leg_uncertainty[loop_leg_tracked];

        ROS_INFO("track[%i]: (%f, %f)", loop_leg_tracked, leg_tracked[loop_leg_tracked].x, leg_tracked[loop_leg_tracked].y);
        ...
        for (int loop_leg_detected=0; loop_leg_detected<nb_legs_detected; loop_leg_detected++)
            if ( distancePoints(leg_tracked[loop_leg_tracked], leg_detected[loop_leg_detected]) < leg_uncertainty[loop_leg_tracked] ) {
                dist_association[loop_leg_tracked][nb_possible_association[loop_leg_tracked]] = distancePoints(leg_tracked[loop_leg_tracked], leg_detected[loop_leg_detected]);
                possible_association[loop_leg_tracked][nb_possible_association[loop_leg_tracked]] = loop_leg_detected;
                nb_possible_association[loop_leg_tracked]++;
                observation_detected[loop_leg_detected]++;
                corresponding_track[loop_leg_detected] = loop_leg_tracked;

                ROS_INFO("detection[%i]: (%f, %f)", loop_leg_detected, leg_detected[loop_leg_detected].x, leg_detected[loop_leg_detected].y);
                /*display[nb_pts] = leg_detected[loop_leg_detected];
...
bool determine_next_association() {

    ROS_INFO("determine_next_association");
    if ( position > 0 )
        for ( int loop_leg_tracked=0; loop_leg_tracked<position; loop_leg_tracked++ )
            index[loop_leg_tracked] = 0;

     //ROS_INFO("index[%i] = %i before next", position, index[position]);
     while ( ( position < nb_legs_tracked ) && ( index[position] == nb_possible_association[position]-1 ) ) {
        index[position] = 0;
        position++;
     }

    bool cond = position < nb_legs_tracked;
    if ( cond )
        index[position]++;
    //ROS_INFO("index[%i] = %i after next", position, index[position]);
    ROS_INFO("next_association is %i", cond);

    return ( cond );

}//determine_next_association

bool check_association() {

    ROS_INFO("check association");
    bool association_checked = ( index[left] == 0 ) || ( index[right] == 0 );
    if ( !association_checked )
        association_checked = distancePoints(leg_detected[possible_association[left][index[left]]], leg_detected[possible_association[right][index[right]]]) < legs_distance_max;        
    if ( association_checked ) {
        position = nb_legs_tracked;
        for ( int loop_leg_tracked=0; loop_leg_tracked<nb_legs_tracked; loop_leg_tracked++ )
            for ( int loop_leg_tracked2=loop_leg_tracked+1; loop_leg_tracked2<nb_legs_tracked; loop_leg_tracked2++ ) {
                bool current_association = ( ( possible_association[loop_leg_tracked][index[loop_leg_tracked]] != possible_association[loop_leg_tracked2][index[loop_leg_tracked2]] ) || ( index[loop_leg_tracked] == 0 ) || ( index[loop_leg_tracked2] == 0 ) );
                if ( ( loop_leg_tracked < position ) && ( !current_association ) )
                    position = loop_leg_tracked;
                association_checked = association_checked && current_association;
            }
    }
    else
        position = 0;
    ROS_INFO("association_checked: %i", association_checked);
    if ( association_checked )
        position = 0;
    //ROS_INFO("position: %i", position);

    return ( association_checked );
...
void associate_person() {
    ...
    while ( cond ) {
        ...
        cond = determine_next_association();
        /*for( int loop_leg_tracked=0; loop_leg_tracked<nb_legs_tracked; loop_leg_tracked++)
            if ( index[loop_leg_tracked] == 0 )
                ROS_INFO("track[%i] is unassociated = %f", loop_leg_tracked, leg_uncertainty[loop_leg_tracked]);
            else
                ROS_INFO("track[%i] is associated with detection[%i] = %f", loop_leg_tracked, possible_association[loop_leg_tracked][index[loop_leg_tracked]], dist_association[loop_leg_tracked][index[loop_leg_tracked]]);
*/
        if ( cond && check_association() ) {
            ...
            for( int loop_leg_tracked=0; loop_leg_tracked<nb_legs_tracked; loop_leg_tracked++)
                if ( index[loop_leg_tracked] == 0 ) {
                    ...
                    current_uncertainty += dist_association[loop_leg_tracked][index[loop_leg_tracked]];
                    ROS_INFO("track[%i] is associated with detection[%i] = %f", loop_leg_tracked, possible_association[loop_leg_tracked][index[loop_leg_tracked]], dist_association[loop_leg_tracked][index[loop_leg_tracked]]);
                }
            ...
            if ( current_uncertainty < best_uncertainty ) {
                for( int loop_leg_tracked=0; loop_leg_tracked<nb_legs_tracked; loop_leg_tracked++)
                    best_association[loop_leg_tracked] = possible_association[loop_leg_tracked][index[loop_leg_tracked]];
                best_uncertainty = current_uncertainty;
                ROS_INFO("this is the best");
                /*if ( nb_legs_tracked >= 25 ) {
...
void manage_track() {
    ...
    for (int loop_leg_tracked=0; loop_leg_tracked<nb_legs_tracked; loop_leg_tracked++)
        if ( best_association[loop_leg_tracked] != unassociated ) {
            detection_associated[best_association[loop_leg_tracked]] = true;
            leg_tracked[loop_leg_tracked] = leg_detected[best_association[loop_leg_tracked]];
            ...
            ROS_INFO("track [%i]:(%f, %f) has been associated with detection[%i]: (%f, %f), frequency = %i", loop_leg_tracked, leg_tracked[loop_leg_tracked].x, leg_tracked[loop_leg_tracked].y, best_association[loop_leg_tracked], leg_detected[best_association[loop_leg_tracked]].x, leg_detected[best_association[loop_leg_tracked]].y, leg_frequency[loop_leg_tracked]);
        }
        else {
    ...
    for (int loop_leg_detected=0; loop_leg_detected<nb_legs_detected; loop_leg_detected++)
        if ( !detection_associated[loop_leg_detected] && ( ( distancePoints(leg_detected[loop_leg_detected], leg_tracked[left]) < 2*uncertainty_max_leg ) || ( distancePoints(leg_detected[loop_leg_detected], leg_tracked[left]) < 2*uncertainty_max_leg ) ) ) {
...
void estimate_person() {
    ...
    person_tracked = leg_frequency[left] > 0;
