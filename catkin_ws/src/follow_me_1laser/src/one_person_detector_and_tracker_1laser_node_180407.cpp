    ...
    for (int loop_leg1=0; loop_leg1<nb_legs_detected; loop_leg1++)
        for (int loop_leg2=loop_leg1+1; loop_leg2<nb_legs_detected; loop_leg2++)
            if ( distancePoints(leg_detected[loop_leg1], leg_detected[loop_leg2]) < legs_distance_max ) {
                ...
                leg1[nb_persons_detected] = loop_leg1;
                leg2[nb_persons_detected] = loop_leg2;
    ...
    for (int loop=0; loop<nb_persons_detected; loop++)
        if ( person_dynamic[loop] )
        {
            ...
            for(int loop_hit=0; loop_hit<nb_beams;loop_hit++)
                if ( cluster[loop_hit] == leg1[loop] || cluster[loop_hit] == leg2[loop] )
                {
...
void track_moving_person(bool d) {
    ...
    for( int current_loop=0; current_loop<nb_beams; current_loop++ )
    {
        ...
        int indice_min = 0;
        float dist_min = uncertainty_min_leg;
        int label_min = unassociated;

        if ( d )
            ROS_INFO("[0][%i] associated with", current_loop);
        ...
        if ( d && label_min == leg )
            ROS_INFO("[0][%i] = %f => leg", indice_min, dist_min);
        if ( d && label_min != leg )
            ROS_INFO("[0][%i] = %f => no leg", indice_min, dist_min);
        current_label[current_loop] = label_min;
            ...
            if ( current_label[current_loop] != unassociated ) {
                ...
                goal_to_reach.x += current_scan[current_loop].x;
                goal_to_reach.y += current_scan[current_loop].y;
                nb_pts++;
            }
    ...
    /*    ROS_INFO("press enter to continue");
        ...
        moving_person_detected = nb_pts>0;
        if ( moving_person_detected )
        {

            goal_to_reach.x /= nb_pts;
            goal_to_reach.y /= nb_pts;

            pub_perception.publish(goal_to_reach);

        }
        ROS_INFO("match_scan done");
