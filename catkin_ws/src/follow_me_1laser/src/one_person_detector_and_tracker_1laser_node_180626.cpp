...
void update() {
    ...
    if ( new_laser && new_robot && init_odom ) {
        if ( data_from_bag ) {
            new_laser = false;
            new_robot = false;
        }
...
//else we start a new cluster
    ...
    cluster[0] = nb_cluster;// the first hit belongs to the first cluster
    cluster_start[nb_cluster] = 0;// the first hit is the start of the first cluster
    cluster_end[nb_cluster] = 0;// the first hit is the end of the first cluster
    nb_dynamic[nb_cluster] = dynamic[0];// to count the number of hits of the current cluster that are dynamic
    nb_pts[nb_cluster] = 1;
    nb_cluster++;

    for( int loop_beam=1; loop_beam<nb_beams; loop_beam++ ) {
        bool found = false;
        int cluster_found;
        for( int loop_beam2=0;loop_beam2<loop_beam; loop_beam2++ )
            if ( ( !found ) && ( distancePoints(current_scan[loop_beam], current_scan[loop_beam2]) < cluster_threshold ) ) {
                cluster_found = cluster[loop_beam2];
                found = true;
            }
        if ( ! found ) {
            cluster[loop_beam] = nb_cluster;
            cluster_start[nb_cluster] = loop_beam;
            cluster_end[nb_cluster] = loop_beam;// the first hit belongs to the first cluster
            nb_dynamic[nb_cluster] = dynamic[loop_beam];// to count the number of hits of the current cluster that are dynamic
            nb_pts[nb_cluster] = 1;
            nb_cluster++;
        }
        else {            
            cluster[loop_beam] = cluster_found;
            if ( distancePoints(current_scan[cluster_start[cluster_found]], current_scan[loop_beam]) > distancePoints(current_scan[loop_beam], current_scan[cluster_end[cluster_found]]) ) {
                if ( distancePoints(current_scan[cluster_start[cluster_found]], current_scan[loop_beam]) > distancePoints(current_scan[cluster_start[cluster_found]], current_scan[cluster_end[cluster_found]]) )
                    cluster_end[cluster_found] = loop_beam;
            }
            else
                if ( distancePoints(current_scan[loop_beam], current_scan[cluster_end[cluster_found]]) > distancePoints(current_scan[cluster_start[cluster_found]], current_scan[cluster_end[cluster_found]]) )
                    cluster_start[cluster_found] = loop_beam;

            nb_dynamic[cluster_found] += dynamic[loop_beam];
            nb_pts[cluster_found]++;
        }
    }

    for( int loop_cluster=0; loop_cluster<nb_cluster; loop_cluster++ ) {
        cluster_dynamic[loop_cluster] = nb_dynamic[loop_cluster]*100 / nb_pts[loop_cluster];
        cluster_size[loop_cluster] = distancePoints(current_scan[cluster_start[loop_cluster]], current_scan[cluster_end[loop_cluster]]);
        cluster_middle[loop_cluster].x = (current_scan[cluster_start[loop_cluster]].x+current_scan[cluster_end[loop_cluster]].x)/2;
        cluster_middle[loop_cluster].y = (current_scan[cluster_start[loop_cluster]].y+current_scan[cluster_end[loop_cluster]].y)/2;
        cluster_middle[loop_cluster].z = (current_scan[cluster_start[loop_cluster]].z+current_scan[cluster_end[loop_cluster]].z)/2;

        ROS_INFO("cluster[%i](%f, %f): [%i](%f, %f) -> [%i](%f, %f), size: %f, dynamic: %i, %f", loop_cluster,
                                                                                 cluster_middle[loop_cluster].x,
                                                                                 cluster_middle[loop_cluster].y,
                                                                                 cluster_start[loop_cluster],
                                                                                 current_scan[cluster_start[loop_cluster]].x,
                                                                                 current_scan[cluster_start[loop_cluster]].y,
                                                                                 cluster_end[loop_cluster],
                                                                                 current_scan[cluster_end[loop_cluster]].x,
                                                                                 current_scan[cluster_end[loop_cluster]].y,
                                                                                 cluster_size[loop_cluster],
                                                                                 nb_dynamic[loop_cluster],
...
void search_local_minimum() {
    ...
    for(int loop=0; loop<nb_cluster; loop++) {
        if ( loop == 0 )
            local_minimum[loop] = ( distancePoints(origin, current_scan[cluster_end[loop]]) < distancePoints(origin, current_scan[cluster_start[loop+1]]) - 0.1 );
        else
            if ( loop == nb_cluster-1 )
                local_minimum[loop] = ( distancePoints(origin, current_scan[cluster_start[loop]]) < distancePoints(origin, current_scan[cluster_end[loop-1]]) - 0.1 );
            else
                local_minimum[loop] = ( distancePoints(origin, current_scan[cluster_end[loop]]) < distancePoints(origin, current_scan[cluster_start[loop+1]]) - 0.1 ) &&
                                             ( distancePoints(origin, current_scan[cluster_start[loop]]) < distancePoints(origin, current_scan[cluster_end[loop-1]]) - 0.1 );
    }
...
void predict_person() {
    ...
    ROS_INFO("diff_orientation: %f", diff_orientation);
    if ( diff_orientation ) {
...
void manage_track() {
    ...
    for (int loop_leg_detected=0; loop_leg_detected<nb_legs_detected; loop_leg_detected++)
        if ( !best_detection[loop_leg_detected] ) {
            ...
            /*if ( ( distancePoints(leg_tracked[nb_legs_tracked], leg_tracked[left]) < legs_distance_min ) ||
                ( distancePoints(leg_tracked[nb_legs_tracked], leg_tracked[right]) < legs_distance_min ) )
                ROS_WARN("track too close from person");*/
            nb_legs_tracked++;
        }
