...
start_swerve = -40	#Start swerve
end_swerve = 40		#End swerve
step_swerve = 5		#Step swerve

swerve_tab = []		#X tab
swerve_error_tab = []	#Y tab

#################
#   Variables   #
#################

swerve = start_swerve					#Set the actual swerve
...
while(True):	#Infinite loop
    ...
    if(isinstance(ids,np.ndarray)):		#If marker detected:
        ...
        value = tvec[0]*100	#Get the swerve
    ...
    if(result == ord('s')):	#If s pressed
        ...
        if(isinstance(ids,np.ndarray)):		#If marker detected:
            ...
            print("Measured swerve = " + str(value) + "cm for "+ str(swerve) + "cm.")	#Log info
            swerve_error_tab.append(abs(swerve-value))					#Save error
            swerve_tab.append(swerve)							#Save swerve
            
        else:	#If no marker detected
            print("No marker detected for " + str(swerve) + "cm.")	#Log info

        swerve += step_swerve		#Incrementation
            
        if(swerve > end_swerve):	#If finish
            break			#Break
...
plt.plot(swerve_tab,swerve_error_tab,'ro')			#Create a graph
plt.xlabel('Ecart reel (en cm)')				#Set x label
plt.ylabel('Erreur absolue sur l\'ecart mesuree (en cm)')	#Set y label
plt.grid()	#Set grid
plt.show()	#Show graph
